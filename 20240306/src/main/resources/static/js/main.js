...
$(document).ready(function() {
	...
	$('#pswd1').on('input',function(){
		...
		if(passwordPattern.test(inputValue)){
			...
			// 만약 1개 이상 입력하세요 빨간색으로 글씨 띄우기
			$('#displayPw').text('대소문자,특수문자,숫자 1개 이상 입력하세요!');
	...
	// 가입하기 버튼을 누르면 input창에 있는 모든 내용을 가져오기
	$("#btnJoin").on('click', function(){
		console.log("가입하기버튼누름!");
		// 1. input창 모든 값 가져오기
		var userInfo = {
...
// 모달창을 닫는 함수
// 만약 close버튼을 클릭했을 때 안 닫히는 경우
function closeModal() {
	$('#myModal').modal('hide');
}
// 모달창을 보여주는 함수, 가입완료시 메시지 출력으로 모달창을 띄움
